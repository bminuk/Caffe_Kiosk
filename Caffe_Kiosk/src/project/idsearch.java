...
public class idsearch extends JFrame implements ActionListener{
	...
	ImageIcon idsearchicon, searchicon, searchclickicon;
	...
	public idsearch() {
		...
		idsearchicon = new ImageIcon("images/idsearch.jpg");
		...
		JPanel jp = new JPanel() {
			public void paintComponent(Graphics g) {
				g.drawImage(idsearchicon.getImage(), 0, 0,null);
				setOpaque(false);
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(obj == searchbtn) {
			String businessNumber = tfnum.getText();
			
			rs = DB.getRs("select id from mint.manager where businessnumber = '" + businessNumber +"'");
			try {
				rs.next();
			...
			try {
				...
				JOptionPane.showMessageDialog(null, "가입된 사업자번호가 아닙니다.");
			}
